...
def send_pose(pub, pose, duration=1.5, wait=True):
    ...
    if pub.get_num_connections() < 1:
        raise RuntimeError("servo command topic has no subscriber")
    pub.publish(MultiRawIdPosDur(id_pos_dur_list=items))
    if wait:
        import rospy
        from hiwonder_servo_msgs.msg import ServoStateList

        targets = dict((int(sid), int(pose[key])) for key, sid in JOINT_IDS if key in pose)
        feedback_deadline = time.time() + float(duration) + 2.0
        last_errors = {}
        while time.time() < feedback_deadline:
            remaining = max(0.1, feedback_deadline - time.time())
            try:
                msg = rospy.wait_for_message(
                    servo_state_topic(), ServoStateList, timeout=min(0.5, remaining)
                )
            except Exception:
                continue
            actual = dict((int(st.id), int(st.position)) for st in msg.servo_states)
            if not all(sid in actual for sid in targets):
                continue
            last_errors = dict(
                (sid, abs(actual[sid] - target)) for sid, target in targets.items()
            )
            if max(last_errors.values() or [0]) <= 25:
                return
        raise RuntimeError("servo did not reach pose; pulse errors=%s" % last_errors)
