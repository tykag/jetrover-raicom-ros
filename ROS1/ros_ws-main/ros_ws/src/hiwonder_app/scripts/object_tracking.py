...
class ObjectTracker:
    def __init__(self, color, node):
        ...
        self.weight_sum = 1.0
        self.image_proc_size = (320, 180)
        self.stop_position = (320, 270)

    def __call__(self, image, result_image, threshold):
        img_h, img_w = image.shape[:2]
        twist = geo_msg.Twist()
        image = cv2.resize(image, self.image_proc_size)
        ...
        if circle is not None:
            self.lost_target_count = 0
            (x, y), r = circle
            x = x / self.image_proc_size[0] * img_w
            y = y / self.image_proc_size[1] * img_h
            r = r / self.image_proc_size[0] * img_w
            cv2.line(result_image, (self.stop_position[0] - 10, self.stop_position[1]), (self.stop_position[0] + 10, self.stop_position[1]), (255, 255, 0), 2)
            cv2.line(result_image, (self.stop_position[0], self.stop_position[1] - 10), (self.stop_position[0], self.stop_position[1] + 10), (255, 255, 0), 2)
            result_image = cv2.circle(result_image, (int(x), int(y)), int(r), (255 - self.target_rgb[0],
                                                                               255 - self.target_rgb[1],
                                                                               255 - self.target_rgb[2]), 2)
            vx = 0
            vw = 0
            if abs(y - self.stop_position[1]) > 20:
                self.pid_dist.update(y - self.stop_position[1])
                twist.linear.x = misc.set_range(self.pid_dist.output, -0.35, 0.35)
            else:
                self.pid_dist.clear()
            if abs(x - self.stop_position[0]) > 20:
                self.pid_yaw.update(x - self.stop_position[0])
