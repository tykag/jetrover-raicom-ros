...
Points2D SlTransform::Transform(const Points2D &data) {
  ...
  for (auto n : data) {
    ...
    if (n.distance > 0) {
      ...
      double y = n.distance * offset_tan_ + offset_y_;
      d = sqrt(x * x + y * y);
    ...
    switch (version_) {
      ...
      case LDType::LD_08:
        if (n.distance == 0) {
          tmp2.push_back(PointData(angle, d, 0));
        } else {
          tmp2.push_back(PointData(angle, d, n.intensity));
        }
        break;
      default:
        tmp2.push_back(PointData(n.angle, n.distance, n.intensity)); // don't transt
        break;
    }
  }
