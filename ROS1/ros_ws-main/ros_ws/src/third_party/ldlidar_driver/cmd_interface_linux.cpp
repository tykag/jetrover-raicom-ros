...
bool CmdInterfaceLinux::Open(std::string &port_name, uint32_t com_baudrate) {
  ...
  if (-1 == com_handle_) {
    std::cout << "[ldrobot] CmdInterfaceLinux::Open open error! errno:" << errno << std::endl;
    return false;
  }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
    std::cout << "[ldrobot] TCGETS2 first error! errno:" << errno << std::endl;
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_WRITE, 'T', 0x2B, sizeof(struct asmtermios::termios2)), &options)) {
    std::cout << "[ldrobot] TCSETS2 error! errno:" << errno << std::endl;
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
    std::cout << "[ldrobot] TCGETS2 second error! errno:" << errno << std::endl;
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  std::cout << "[ldrobot] actual BaudRate reported: " << options.c_ospeed << std::endl;
